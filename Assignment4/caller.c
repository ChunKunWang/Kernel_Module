 ...
 * the mysync kernel module, requests the pid of the calling
 * process, and outputs the result.  It also outputs the 
 * result from the regular Linux mysync() system call so the
...
void main (int argc, char* argv[])
{
  ...
  my_pid = getpid();
  fprintf(stdout, "System call mysync() returns %d\n", my_pid);

  // use the kernel module to get the pid
  do_syscall(input);

  fprintf(stdout, "Module mysync returns %s", resp_buf);
